$(document).ready( () => {
    ...
    $(".profile_photo").change( function (e) {
        ...
        $(".user_profile_photo").attr("data-src", $(".user_profile_photo").attr("src"))
        setTimeout( () => {
            $(".user_profile_photo").attr("src",  URL.createObjectURL(e.target.files[0])) 
            $(".user_profile_photo").removeClass("skeleton-image")
            const parent = $(".confirm_profile_change")
            const child = $(".confirm_profile_change_main") 
            child.addClass(child.attr("animate-in")) 
            parent.addClass("flex") 
            parent.removeClass("hidden")
            setTimeout( () => {
    ...
    $(".confirm_profile_change").find(".confirm_profile_change_btn").click( async function (e) {
        ...
        if(!action2){
            if(confirm_photo) {
                ...
                action2 = true 
                let data = new FormData()
                data.append('profilePhoto', $(".profile_photo")[0].files[0])
                $.ajax({
                    xhr: function() {
                        var xhr = new window.XMLHttpRequest()
                        xhr.upload.addEventListener("progress", function(e) {
                    ...
                    success: (res) => {
                        ...
                        toast.fire({
                            ...
                            //close cover photo promt 
                            const cover_parent = $(".confirm_profile_change")
                            const cover_child = $(".confirm_profile_change_main")
                            cover_child.addClass(cover_child.attr("animate-out")) 
                            setTimeout( () => {
                                cover_child.removeClass(cover_child.attr("animate-out")) 
                                cover_parent.addClass("hidden") 
                ...
                setTimeout( () => {
                    ...
                    $(".user_profile_photo").addClass("skeleton-image")
                    setTimeout( () => {
                        $(".user_profile_photo").attr("src", $(".user_profile_photo").attr("data-src"))
                        $(".profile_photo").val('')
                        $(".user_profile_photo").removeClass("skeleton-image")
                    }, 500) 
                }, 500)
